            ...
            <div class="q-pa-sm q-gutter-sm">
              ...
              >
                ...
                <template v-slot:top-left>
                  <q-btn v-if="verListado" color="primary" icon="list" label="Mostrar Listado de Documentos" @click="getCategorias" no-caps></q-btn>  
                  <q-btn v-if="!verListado" color="primary" icon="add_circle" label="Agregar Documento" @click="nuevoRow" no-caps></q-btn>                  
                  <div class="q-pa-sm q-gutter-sm">
                          ...
                          <div class="row">
                            ...
                            <div class="col-sm-6 col-xs-12 q-pa-sm q-gutter-sm">
                              <q-input outlined 
                                ...
                                @update:modelValue="val => editedItem.descripcion = val.toUpperCase()"
                                maxlength="250"
                              >
                              </q-input>
                            </div>
                            ...
                            <div class="col-sm-6 col-xs-12 q-pa-sm ">
                              <div class="text-weight-bolder">Se pueden subir hasta 5 documentos o imagenes por publicación.</div>
                            </div>
                            <div class="col-sm-6 col-xs-12 q-pa-sm ">    
...
export default defineComponent({
  ...
  setup() {
    ...
    columnas.value = [
      {name: 'titulo', label: 'Título/Nombre de documento', field: 'titulo', align: 'left', sortable: true  },
      {name: 'descripcion', label: 'Descripción del documento', field: 'descripcion', align: 'left', sortable: true  },   
    ...
    const addRow = async ()=>{
      try {
        if (editedIndex.value > -1) {          
          const docs = await api.put(`api/updates/${editedItem.value.id}/11`, editedItem.value);        
          ...
          const docs = await api.put(`api/inserts/11`, editedItem.value);        
          await Promise.all([docs]).then(function (res) {
              const egr = res[0].data.data
              //console.log(egr[0]); 
              if(formArch.length>0){
    ...
    const deleteItem = async (item)=>{
      ...
      try {
        ...
        $q.dialog({
            ...
            const calle = await api.put(`api/updates/${item.id}/11`, {activo:0,visible:0,deletedAt:deletedAt})     
    ...
    const getCategorias = async () => {   
        ...
        try {                
            const json = await api.post('api/selectsadmin/11', payload);
    ...
    const addDocs= async (idDoc, tipo)=>{
      try {       
        ...
        const allDocs = formArch.map(async (doc) => {
            ...
            const docs = await api.post(`api/uploads/11/${idDoc}/documentos/${doc.ruta}`, formData)             
            return docs.data;
        });
    ...
    const verDoc= async (arr,doc,id) => { 
      ...
      try {        
          ...
          const json = await api.get(`api/uploads/11/${arr.id}/documentos/ruta${id}`,{responseType: 'blob' });
          const {data}=json
    ...
    const borraDoc= async (arr,id) => {  
      console.log(arr,id)
      try {
         const docs = await api.delete(`api/uploads/11/${arr.id}/documentos/ruta${id}`) 
         await Promise.all([docs]).then(function (res) {
              const egr = res[0].data.data
              switch ( id ) {
                case 1:
